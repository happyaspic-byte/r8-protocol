...
Before a valid cookie, the responder has no session allocation and performs no signature verification or X25519. Cookie binding and all prevalidation limits are normative in `spec/parameters-v0.1.md`: `P-COOKIE-BUCKET` through `P-PREVALIDATION-GLOBAL-REFILL`. In particular, `VERIFY_COOKIE` is bounded by `P-VERIFY-COOKIE-MAX-RESPONSE`; the fixed `P-PREVALIDATION-SOURCE-TABLE-MAX` table, fixed-window `P-PREVALIDATION-CUMULATIVE-RATIO`, reject-new table-full rule, and global token bucket prevent amplification. Missing capacity silently drops as `E-COOKIE`.

Post-cookie work remains bounded by `P-PENDING-SESSIONS-MAX`, `P-ESTABLISHED-SESSIONS-MAX`, `P-OPEN-TOTAL-TIMEOUT`, `P-POST-COOKIE-PENDING-TIMEOUT`, and `P-SESSION-IDLE-TIMEOUT`. Reject-new behavior is deterministic and never evicts established state. The source table, session table, candidate records, path state, replay/dedup caches, and queues have no attacker-controlled unbounded growth; their limits and `E-CAPACITY` behavior are registry-defined.

## Transcript, nonce, counter, and replay

The canonical session transcript includes both EIDs and full keys, roles, intended responder/service context, SCID, versions/profile, ephemerals, and nonces. Session keys are separated by the exact per-direction/profile/path-slot HKDF schedule in `spec/0005-session-security-v0.1.md`; its nonce is exactly four zero bytes followed by the big-endian `u64` counter. Counter zero is forbidden, concurrent sends atomically reserve a counter, and exhaustion follows `P-SESSION-COUNTER-MAX` and `P-COUNTER-USABLE-RANGE`. Restart discards sessions, replay/candidate/path state, counters, and ephemeral keys and creates a fresh 16-byte boot instance under the same session-security specification; it never resumes an old key domain.
