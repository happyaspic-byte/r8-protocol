...
5. Run `make compare-smoke`. Comparison adapters remain fail-closed scaffolding: `publication_eligible=false` until real protocol transfers and trusted provenance verification are implemented.
6. Run `python3 bench/repro.py --output repro.json`. The bundle may hash supplied third-party evidence, but `independent_reproducer` remains false until identity is verified outside the project tool.
