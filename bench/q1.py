...
def regenerate(output):
    ...
    if environment.get("source_identity") != manifest.get("source_identity") or environment.get("implementation_sources") != implementation_sources() or manifest.get("implementation_sources") != implementation_sources() or environment.get("source_identity") != (manifest.get("source_identity")) or (not json.loads((output/"smoke_non_result.json").read_text()) and environment.get("source_identity") != source_identity()) or environment.get("git_commit") != manifest.get("git_commit") or environment.get("host_epoch") != manifest.get("host_epoch") or environment.get("configuration_sha256") != expected_config or environment.get("binary_sha256") != source_digest() or environment.get("interface_count") != 10 or environment.get("non_loopback_interface_count") != 7 or environment.get("veth_pair_count") != 3 or topology.get("topology_sha256") != topology_digest() or topology.get("configuration_sha256") != expected_config or topology.get("interface_count") != 10 or topology.get("non_loopback_interface_count") != 7 or topology.get("veth_pair_count") != 3: raise RuntimeError("environment binding mismatch")
